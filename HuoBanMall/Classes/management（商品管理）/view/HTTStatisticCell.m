...
- (void)setModel:(HTStatisticsModel *)model{
    ...
    _model = model;
    UIImage * placeIcon = [UIImage imageNamed:@"zchyzrs"];;
    if(model.Type == 1){
        _dateType.text = @"总返利";
        ...
        self.firstLable.text = [NSString stringWithFormat:@"%@\n购买%d单",model.name,[model.amount integerValue]];
        _datelable.text = [model.money stringValue];
    }else{
        _dateType.text = @"消费额(元)";
        self.firstLable.text = model.orderNo;
        _datelable.text = [model.money stringValue];
    }
//    _datelable.text = [model.num stringValue];
    
    [self.iconView sd_setImageWithURL:[NSURL URLWithString:model.pictureUrl] placeholderImage:placeIcon options:SDWebImageRetryFailed];
    
    if (model.time>0) {
        self.secondLable.text = [self dateFormate:model.time];
    }else{
        self.secondLable.text = nil;
...
//    self.accessoryView.frame = CGRectMake(self.frame.size.width-60, 0, 60,self.frame.size.height);
